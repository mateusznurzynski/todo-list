...
function renderTodoForm(project, initial) {
  ...
  todoFormButtonElement.addEventListener('click', () => {
  ...
  todoFormCancelElement.addEventListener('click', () => {
    todoFormWrapper.innerHTML = '';
...
function refreshTodoElementContent(todo, todoElement) {
  ...
  const todoBasicInfoElement = todoElement.querySelector('.basic-info-text');
...
function initTodoEdit(todo, project, initial, todoElement) {
  ...
  const modalHeaderElement = todoModalElement.querySelector('.modal-header');
  const modalBodyElement = todoModalElement.querySelector('.modal-body');
  const modalFooterElement = todoModalElement.querySelector('.modal-footer');
  ...
  dateCheckboxElement.addEventListener('input', () => {
...
function renderTodos(project, initial) {
  ...
  todos.forEach((todo) => {
    ...
    todoCollapseButton.setAttribute('data-bs-target', `#collapseTodo${todoId}`);
    ...
    const todoDeleteButton = createDomElement('div', 'todo-delete-btn', '');
    ...
    const todoCompleteButton = createDomElement('div', 'todo-complete-btn', '');
    todoCompleteButton.addEventListener('click', () => {
      Project.completeTodo(project.getName(), todo.getName(), initial);
      refreshTodoElementContent(todo, todoElement);
    });
    const todoEditButton = createDomElement('div', 'todo-edit-btn', 'Edit');
    ...
    todoBasicInfoElement.insertAdjacentElement('afterbegin', todoDeleteButton);
...
function renderProject(projectName, initial) {
  ...
  if (!initial) {
    ...
    projectEditElement.addEventListener('click', () => {
...
function refreshMainElement(projects) {
  ...
  const project = projects.find(
    (element) => element.getName() === activeProject.name
  );

  if (project) {
    renderProject(project.getName());
  } else {
    renderProject(DEFAULT_PROJECT.name, DEFAULT_PROJECT.type === 'initial');
  }
  return true;
}
...
function initPage() {
  ...
  PubSub.subscribe('todosChanged', () => {
    clearMainElement();
    renderProject(activeProject.name, activeProject.type === 'initial');
  });
  loadImages();
  addListeners();
